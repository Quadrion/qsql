...
namespace qtl
{
	...
	namespace qinternal
	{
		...
		{
		public:
			forward_vector_iterator(T* ptr = nullptr, const std::size_t offset = 0);
		...
		template<typename T>
		inline forward_vector_iterator<T>& forward_vector_iterator<T>::operator--()
		{
			--ptr;
			return *this;
		}
	...
	public: // member methods
		...
		forward_iterator<T> emplace(forward_iterator<T> pos, Arguments && ... args);
		...
		/// Value to insert
		/// </param>
		forward_iterator<T> insert(forward_iterator<T> pos, const T& value);
		...
		/// Value to insert
		/// </param>
		forward_iterator<T> insert(forward_iterator<T> pos, T&& value);
	...
	inline bool vector<T>::operator==(const vector<T>& other) const
	{
		if (__size == other.__size)
		{
			...
			{
				if (static_cast<T*>(__data)[i] != static_cast<T*>(other.__data)[i])
				{
					return false;
				}
			}
	...
	inline void vector<T>::resize(const std::size_t sz)
	{
		if (sz > __size)
		{
			if (sz > __capacity) __expand_capacity(sz);
	...
	inline void vector<T>::push_back(T && value)
	{
		...
		assert(__data);
		T* data = static_cast<T*>(__data);
		::new (data + __size) T(qtl::move(value));
		__size++;
	}

	template<typename T>
	template<typename ...Arguments>
	inline typename vector<T>::template forward_iterator<T> vector<T>::emplace(forward_iterator<T> pos, Arguments && ...args)
	{
		...
		__size++;
		return vector<T>::forward_iterator<T>(__data + idx, __size);
	}
	...
	template<typename T>
	inline typename vector<T>::template forward_iterator<T> vector<T>::insert(forward_iterator<T> pos, const T & value)
	{
		std::size_t idx = &pos.operator*() - __data;
		if (__size == __capacity)
		{
	...
	template<typename T>
	inline typename vector<T>::template forward_iterator<T> vector<T>::insert(forward_iterator<T> pos, T && value)
	{
	...
	inline void vector<T>::__expand_capacity(const std::size_t capacity)
	{
		if (__data)
		{
			auto tmp = static_cast<T*>(realloc(__data, sizeof(T) * capacity));
			if (tmp)
			{
				__data = tmp;
			}
		...
		{
			__data = static_cast<T*>(malloc(sizeof(T) * capacity));
		}
		__capacity = capacity;
	}
