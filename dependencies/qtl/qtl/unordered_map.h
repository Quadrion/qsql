...
namespace qtl
{
	...
	namespace qinternal
	{
		...
		constexpr size_t fast_mod2(const size_t left, const size_t right)
		{
			return left & (right - 1);
		}
	} // qinternal
	...
	template<typename Key, typename Value, typename Hash, std::size_t DefaultSize>
	inline void unordered_map<Key, Value, Hash, DefaultSize>::reserve(const std::size_t newSize)
	{
		if (newSize > __usage)
			__resize(newSize);
	}
		...
		for (std::size_t i = 0; i < __capacity; i++)
		{
			...
			if (__data[idx].is_occupied)
			{
				...
				if (__data[next_idx].is_occupied && __data[next_idx].kv.first == key)
				{
					...
					{
						return unordered_map<Key, Value, Hash, DefaultSize>::forward_iterator<Key, Value>(__data, next_idx, __capacity);
					}
				}
			}
			else
